...
void Comb_filter::update_log_intensity_state(const double& ts,
    const int& x, const int& y, const bool& polarity)
{
  ...
  log_intensity_state_.at<double>(y, x) += contrast_threshold;
  // ts_array_.at<double>(y, x) = ts;

}

// void Comb_filter::update_log_intensity_state_global(const double& ts)
// {
//   cv::Mat beta;
...
void Comb_filter::update_leaky_event_count(const double& ts, const int& x, const int& y,
    const bool& polarity)
{
  if (polarity)
  {
...
void Comb_filter::minMaxLocRobust(const cv::Mat& image, double* robust_min, double* robust_max,
